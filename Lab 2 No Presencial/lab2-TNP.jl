...
function get_lp()
  ...
  @constraint(mdl, [i=1:n-1], x[n*n + i + 1] <= x[n*n + i]) # Romper la simetría de los colores
  @constraint(mdl, x[1] == 1) # En el óptimo, el vértice 1 estará coloreado de algún color, 
                                # lo fijamos desde un inicio y se rompe una simetría
  @constraint(mdl, x[n*n + 1] == 1) # Se usó el color 1 en el primer vértice
  @objective(mdl, Min, sum(x[n*n+i] for i=1:n ))

  return mdl, all_variables(mdl)
end

function get_var_score(var::Int64, z::Float64, current_node::BBNode)
...
function get_root_incumbent()
  ...
  function color_vertex(vertex, graph, colored, colors)
    # Función que colorea un vértice tal que el color usado sea el mínimo de
    # los colores no usados por sus vecinos
    ...
    forbidden = Set(colored[neighbor] for neighbor in neighbors if !(colored[neighbor]==0))
    possible = setdiff(Set(1:colors), forbidden)
    if isempty(possible)
      colors = colors+1
      colored[vertex] = colors
    else
      colored[vertex] = minimum(possible)
    end
    return colored, colors
  end

  graph = create_graph(E)
  colored = Dict(i => 0 for i in 1:n)
  colors = 0
  for vertex in keys(graph)
    if colored[vertex] == 0
      colored, colors = color_vertex(vertex,graph,colored,colors)
    end
  end
  for vertex in keys(colored)
    x[n*(vertex-1) + colored[vertex]] = 1
    x[n*n + colored[vertex]] = 1
  end
  return x, maximum(values(colored))
end

function create_graph(E)
  # Función que retorna un diccionario en que está almacenado el grafo
  n = maximum(E)
  graph = Dict(i => [] for i in 1:n)
  for w in eachrow(E)
    push!(graph[w[1]], w[2])
    push!(graph[w[2]], w[1])
  end
...
function get_CCs(graph)
  # Función que retorna las componentes conexas de un grafo a partir de
  # una representación en Dict
  visited = Set()
  components = Set()
  for vertex in keys(graph)
    if !(vertex in visited)
      push!(visited,vertex)
      CC = Set([vertex])
      CC = get_CC_from_vertex(vertex,graph,CC,visited)
      push!(components, CC)
    elseif isempty(setdiff(keys(graph), visited))
      break
    else
    end
  end
