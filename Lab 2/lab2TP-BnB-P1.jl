...
function bnb()
  ...
  if solution_is_boolean(map(x->value(x),root_vars))
    ...
    status = BBStatus(root_obj, root_obj, map(x->value(x),root_vars), root, root_vars)
    return status
  end
...
#    println("out [" * string(n_nodes_explored) * "] Exploring one more node.")
    ...
    # End iteration.
    if ( n_nodes_explored < MAX_SHOW_EXPLORED )
      println("out [" * string(n_nodes_explored) * "] Gap: " * string(gap(status)) * ". Bnd: [" * string(status.lower_bound) * "; " * string(status.upper_bound) * "].")
    end
  end
